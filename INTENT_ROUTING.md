...
## 12. data_analysis 查询计划管线

当前 `sql_query` 已升级为数据分析管线，外部仍保留 `skill = sql_query`，内部先生成 Query Plan，再由后端编译安全 SQL。
...
自然语言问题
  ...
  -> 执行查询
  -> 后处理
  -> 输出 summary/kpi/chart/table/followups blocks
```

### 12.2 当前支持的分析类型

- `trend`：趋势分析，例如“5 月某客户每日消耗趋势”
- `ranking`：排名分析，例如“5 月消耗最高的 10 个客户”
- `intersection_comparison`：交集维度对比，例如“两个客户投放相同品牌的消耗对比”
...
{
  "analysis_type": "trend",
  "time_range": { "value": "2026-05" },
  "filters": [{ "field": "customer_name", "op": "contains", "value": "车商通服务商" }],
  "dimensions": ["stat_date", "stat_month", "customer_name"],
  "metrics": [{ "name": "cost_amount" }],
...
```text
分析5月份车商通服务商和北方智投服务商投放相同品牌的广告消耗金额对比
```

系统会识别为 `intersection_comparison`，先找两个客户共同投放的品牌，再输出共同品牌消耗对比表和图表。

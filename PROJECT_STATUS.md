...
### 4.3 数据分析 Query Plan 管线

当前 `sql_query` 外部名称保持不变，但内部已升级为数据分析管线：

```text
自然语言 -> Query Plan -> 安全 SQL -> 后处理 -> BI blocks
```

当前支持：

- 5 月某客户每日消耗趋势，输出折线图
- 5 月消耗最高客户排名，输出柱状图和表格
- 两个客户投放相同品牌的广告消耗对比，输出对比表和柱状图

该能力不再依赖单个硬编码趋势函数，而是使用字段白名单、指标注册表和 Query Plan 编译 SQL。
